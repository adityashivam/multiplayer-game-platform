 ...
 * @param {number} [config.hardSnapDistSq=19600]
 * @param {number} [config.hardSnapVerticalError=100]
 * @param {number} [config.correctionBlendFactor=0.36]
...
export function createClientPredictor(config = {}) {
  ...
  const hardSnapDistSq = config.hardSnapDistSq ?? 19600;
  const hardSnapVerticalError = config.hardSnapVerticalError ?? 100;
  const correctionBlendFactor = config.correctionBlendFactor ?? 0.36;
