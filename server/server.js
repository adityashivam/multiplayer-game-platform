...
// Serve index.html for root and any room URL
app.get("/", (req, res) => {
...
app.get("/api/new-room", (req, res) => {
  ...
  res.json({ roomId, url: `http://${req.get("host")}/room/${roomId}` });
});
...
function createInitialGameState(gameId) {
  games[gameId] = {
    players: {
      p1: {
        x: 200,
        y: GROUND_Y - 42, // Subtract sprite height so feet touch ground
        vx: 0,
        vy: 0,
        dir: 1,
        health: 500,
...
setInterval(() => {
  ...
  for (const [gameId, state] of Object.entries(games)) {
    ...
    io.to(gameId).emit("state", sanitizeStateForClients(state));
  }
}, 1000 / FPS);
...
io.on("connection", (socket) => {
  ...
  socket.on("joinGame", ({ gameId }) => {
    ...
    if (!room) {
      ...
      initAttackTimestamps(games[gameId]);
    } else if (room.size === 1) {
      playerId = "p2";
      socket.join(gameId);
      socket.data.gameId = gameId;
    ...
    console.log(`Socket ${socket.id} joined game ${gameId} as ${playerId}`);
...
httpServer.listen(PORT, () => {
  console.log(`🎮 Kaboom Fight Server running on http://localhost:${PORT}`);
});
